...
% profile on
% profile off
% run('C:\Users\Dell\Documents\GitHub\vlfeat-0.9.21\toolbox\vl_setup.m')
addpath('./flann/');
addpath('./keypointExtraction/');
% addpath('./estimateRigidTransform');
s=100;
gridStep=0.06;% 0.05 0.1
overlap=0.2;
icpSteps=100;
TrMin=0.2;
TrMax=0.8;
...
%% 转灰度
map1=imread('Fr1_3.png','png');%rgb2gray ./testPic/simple4.png
map2=imread('Fr2_5.png','png');
map1=map1(:,:,1);
map2=map2(:,:,1);
% map2=rgb2gray(imread('Fr2_3.png','png'));
% map1 = imresize(map1,1.0);
% map2 = imresize(map1,1.5);
...
%% Laplaceharris(慢matlab)
 cornersM1raw=harrislaplace(map1);
 cornersM2raw=harrislaplace(map2);
%  draw(map1,cornersM1,'m1');
%  draw(map2,cornersM2,'m2');
 figure;imshow(map1);hold on
 showCornerScale(cornersM1raw);
 figure;imshow(map2);hold on
 showCornerScale(cornersM2raw);
 
 cornersM1=SURFPoints(cornersM1raw(:,1:2),'Scale',cornersM1raw(:,3));
 cornersM2=SURFPoints(cornersM2raw(:,1:2),'Scale',cornersM2raw(:,3));
...
seleCorM1=selectUniform(cornersM1,500,size(map1));% seleCorM1=cornersM1.selectStrongest(300);
showPoint(seleCorM1.Location);

figure;
imshow(map2);hold on;
seleCorM2=selectUniform(cornersM2,500,size(map2));
showPoint(seleCorM2.Location);
% 不可用
% zSeleCorM1.Location=seleCorM1.Location/s;
% zSeleCorM1.Scale=seleCorM1.Scale;
% zSeleCorM2.Location=seleCorM2.Location/s;
% zSeleCorM2.Scale=seleCorM2.Scale;
